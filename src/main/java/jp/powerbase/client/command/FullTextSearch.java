/*
 * @(#)$Id: FullTextSearch.java 1178 2011-07-22 10:16:56Z hirai $
 *
...
public class FullTextSearch extends Invoker implements CommandInvoker {
	private final Command command = Command.FULL_TEXT_SEARCH;

	public FullTextSearch(Connector connector, Path database) throws MalformedURLException, NoSuchAlgorithmException, UnsupportedEncodingException {
		this(connector, database, null);
	}

	public FullTextSearch(Connector connector, Path database, Map<String, String> params) throws MalformedURLException, NoSuchAlgorithmException, UnsupportedEncodingException {
		String query = "";
		if (params != null) {
			if (params.size() != 0) {
				query = "?cmd=" + command.value;
