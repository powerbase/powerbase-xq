...
public class RemovePermissionFromDirectory extends Invoker implements CommandInvoker {
	private final Command command = Command.REMOVE_PERMISSION_FROM_DIRECTORY;

	public RemovePermissionFromDirectory(Connector connector, Path directory, Document xml) throws NoSuchAlgorithmException, IOException {
		client = new Client(connector.getUser(), Method.PUT, connector.getServer().addPath(directory).getUrl());
