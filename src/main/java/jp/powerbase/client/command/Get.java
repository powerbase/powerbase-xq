...
public class Get extends Invoker implements CommandInvoker {
	private final Command command = Command.GET;

	public Get(Connector connector, Path database) throws MalformedURLException, NoSuchAlgorithmException, UnsupportedEncodingException {
		this(connector, database, null);
	}

	public Get(Connector connector, Path database, Map<String, String> params) throws MalformedURLException, NoSuchAlgorithmException, UnsupportedEncodingException {
		String query = "";
		if (params != null) {
			if (params.size() != 0) {
